...
from app.schemas.auth import LoginRequest, RefreshRequest, RegisterRequest, TokenPair, PasswordChangeRequest
...
def register(payload: RegisterRequest, request: Request, db: Session = Depends(deps.get_db)):
    email_hash = hash_pii(payload.email)
    if db.query(User).filter((User.username == payload.username) | (User.email_hash == email_hash)).first():
        raise HTTPException(status_code=400, detail="Username or email already exists")
    hashed = security.hash_password(payload.password)
    user = User(username=payload.username, password_hash=hashed, role=UserRole.developer)
...
def login(request: Request, payload: LoginRequest, db: Session = Depends(deps.get_db)):
    try:
        ...
        audit_log("login_failed", None, request.client.host if request.client else None, username=payload.username)
        raise
    access, refresh, expires_in = auth_service.create_token_pair(str(user.id))
    audit_log("login_success", str(user.id), request.client.host if request.client else None)
...
def refresh(payload: RefreshRequest, request: Request):
    ...
    access = security.create_token(user_id, token_type="access", expires_delta=timedelta(minutes=settings.access_token_expire_minutes))
    ...
    auth_service.blacklist_refresh(payload.refresh_token, settings.refresh_token_expire_days * 24 * 3600)
...
def logout(
    ...
    audit_log("logout", data.get("sub"), request.client.host if request.client else None)
    return None
...
def logout_all(request: Request, current_user: User = Depends(deps.get_current_user)):
    ...
    audit_log("logout_all", str(current_user.id), request.client.host if request.client else None)
    return None
...
def change_password(
    ...
    auth_service.revoke_all_refresh(current_user.id)
    auth_service.revoke_all_access(current_user.id)
    audit_log("change_password", str(current_user.id), request.client.host if request.client else None)
    return None
