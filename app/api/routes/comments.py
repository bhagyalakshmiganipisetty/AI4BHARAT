...
from app.schemas.comment import CommentCreate, CommentCreateForIssue, CommentOut, CommentUpdate
...
def list_comments_legacy(issue_id: UUID, db: Session = Depends(deps.get_db), _: User = Depends(deps.get_current_user)):
...
def add_issue_comment(
    ...
    comment = Comment(content=sanitize_markdown(payload.content) or "", issue_id=issue.id, author_id=current_user.id)
    db.add(comment)
    db.commit()
    db.refresh(comment)
    audit_log(
...
def add_comment_legacy(
    ...
    if not issue:
        raise HTTPException(status_code=404, detail="Issue not found")
    comment = Comment(content=sanitize_markdown(payload.content) or "", issue_id=payload.issue_id, author_id=current_user.id)
    db.add(comment)
    db.commit()
    db.refresh(comment)
    audit_log(
