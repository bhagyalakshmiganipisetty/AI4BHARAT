...
def create_token_pair(user_id: str) -> tuple[str, str, int]:
    ...
    access = security.create_token(user_id, token_type="access", expires_delta=access_exp)
    refresh_jti = str(uuid.uuid4())
    refresh = security.create_token(user_id, token_type="refresh", expires_delta=refresh_exp, jti=refresh_jti)
    token_store.store.add_refresh_session(user_id, refresh_jti, int(refresh_exp.total_seconds()))
...
def enforce_refresh_active(user_id: str, jti: str | None) -> None:
    ...
    if not jti or not token_store.store.is_refresh_active(user_id, jti):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Stale refresh token")
...
def rotate_refresh_session(user_id: str, old_jti: str) -> str:
    ...
    token_store.store.rotate_refresh_session(user_id, old_jti, new_jti, int(refresh_exp.total_seconds()))
    return new_jti
...
def revoke_all_access(user_id: str) -> None:
    ttl_seconds = int(timedelta(minutes=settings.access_token_expire_minutes).total_seconds())
